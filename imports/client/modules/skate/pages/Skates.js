...
class Track extends React.Component {
  render() {
    const { match, history, deck, tracking, stop, startTime, _id } = this.props;
    const age = Moment(startTime).fromNow();
    return (
      <ListItem button onClick={() => history.push(`${match.url}/${_id}`)} key={_id}>
        <ListItemText primary={"Deck:" + deck.name + " distance: " + utils.getDistance(tracking) + "m"} secondary={age} />
        <ListItemSecondaryAction>
